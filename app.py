...
.tabs{display:flex;gap:0;margin-bottom:24px;border-bottom:2px solid #e5e9f0}
.tab{flex:1;padding:10px;background:none;border:none;font-size:.9rem;font-weight:600;
     color:#6b7a8d;cursor:pointer;border-bottom:3px solid transparent;margin-bottom:-2px;transition:color .15s}
.tab.active{color:#4f7ef0;border-bottom-color:#4f7ef0}
.tab:hover:not(.active){color:#374151}
.total-box{background:#eef3ff;border:1.5px solid #4f7ef0;border-radius:7px;
           padding:12px 16px;font-size:1.05rem;font-weight:700;color:#1a2a3a;text-align:center}
.total-label{font-size:.78rem;font-weight:600;color:#4f7ef0;margin-bottom:3px}
...
<div class="card">
  ...
  <div class="tabs">
    <button class="tab active" id="tabStandard" onclick="switchTab('standard')">Standard Protocol</button>
    <button class="tab"        id="tabCustom"   onclick="switchTab('custom')">Custom Dose</button>
  </div>
  ...
  <div id="customSection" style="display:none">
    <div class="field">
      <label>Medication</label>
      <input list="cMedList" id="cMedication" placeholder="Select or type medication name"
             autocomplete="off">
      <datalist id="cMedList"></datalist>
    </div>
    <div class="row">
      <div class="field">
        <label>Injection Frequency</label>
        <select id="cFrequency" onchange="calcUnits()">
      ...
      <div class="field">
        <label>Units per Injection</label>
        <input type="number" id="cUnitsPerInj" placeholder="e.g. 50" min="0" step="any" oninput="calcUnits()">
      </div>
    </div>
    <div class="field">
      <div class="total-label">Total Units Needed (4-week period)</div>
      <div class="total-box" id="cTotalUnits">&#8212;</div>
    </div>
    <div class="row">
      <div class="field">
        <label>Patient Name <span style="font-weight:400;color:#9ca3af">(optional)</span></label>
        <input type="text" id="cPatient" placeholder="e.g. Jane Smith">
      </div>
      <div class="field">
        <label>Next Appointment <span style="font-weight:400;color:#9ca3af">(optional)</span></label>
        <input type="text" id="cAppt" placeholder="e.g. 6/15/2026">
      </div>
    </div>
    <button class="btn" id="cGenerateBtn" onclick="generateCustomDoc()">Generate Word Document</button>
    <div id="cStatus"></div>
    <a class="dl-btn" id="cDlBtn" href="#" download>&#11015; Download Document</a>
  </div>
</div>
...
function switchTab(tab) {
  ...
  document.getElementById('standardSection').style.display = isStd ? '' : 'none';
  document.getElementById('customSection').style.display   = isStd ? 'none' : '';
  document.getElementById('tabStandard').classList.toggle('active', isStd);
  document.getElementById('tabCustom').classList.toggle('active', !isStd);
  if (!isStd) populateCustomMed();
}

// ── Populate custom medication datalist ──
function populateCustomMed() {
  const dl = document.getElementById('cMedList');
  if (dl.options.length > 0) return; // already populated
  for (const k of Object.keys(medData)) {
    const o = document.createElement('option');
    o.value = medData[k].display_name;
    dl.appendChild(o);
  }
}
...
function calcUnits() {
  ...
  const units = parseFloat(document.getElementById('cUnitsPerInj').value) || 0;
  const injMap = {'Daily': 28, 'Every Other Day': 14, 'Weekly': 4};
  const total  = units * (injMap[freq] || 0);
  document.getElementById('cTotalUnits').textContent =
    units > 0 ? (Number.isInteger(total) ? total : total.toFixed(2)) + ' units' : '—';
}

// ── Generate custom dose document ──
async function generateCustomDoc() {
  const med   = document.getElementById('cMedication').value;
  const freq  = document.getElementById('cFrequency').value;
  const units = document.getElementById('cUnitsPerInj').value.trim();
  if (!med)   { setCStatus('error','Please select a medication.'); return; }
  if (!units) { setCStatus('error','Please enter units per injection.'); return; }
  const btn = document.getElementById('cGenerateBtn');
  btn.disabled = true; btn.textContent = 'Generating…';
  document.getElementById('cDlBtn').style.display = 'none';
  setCStatus('info','Generating document…');
  try {
    const r = await fetch('/api/generate-custom', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({
        medication: med, frequency: freq,
        units_per_injection: parseFloat(units),
        patient_name: document.getElementById('cPatient').value.trim(),
        appointment:  document.getElementById('cAppt').value.trim()
      })
    });
    if (r.status === 401) { location = '/login'; return; }
    const d = await r.json();
    if (r.ok) {
      setCStatus('success','✓ Document ready — click below to download.');
      const dl = document.getElementById('cDlBtn');
      dl.href = '/download/' + encodeURIComponent(d.filename);
      dl.download = d.filename;
      dl.style.display = 'block';
    } else { setCStatus('error','Error: ' + d.error); }
  } catch(e) { setCStatus('error','Network error. Please try again.'); }
  finally { btn.disabled = false; btn.textContent = 'Generate Word Document'; }
}

function setCStatus(t,m){const e=document.getElementById('cStatus');e.className=t;e.textContent=m;e.style.display='block';}
...
def api_generate_custom():
    ...
    # Injections per week for each frequency option
    freq_inj = {'Daily': 7, 'Every Other Day': 3, 'Weekly': 1}
    inj_week = freq_inj.get(frequency, 7)

    # Build 4 week rows (Every Other Day alternates 4/3 to equal 14 total)
    week_rows = []
    for i in range(4):
    ...
    # The datalist sends display_name text; fall back to matching by display_name
    # (case-insensitive) if the sheet key isn't found directly.
    if med_key not in data:
        matched = next(
            (k for k, v in data.items()
             if v['display_name'].lower() == med_key.lower()),
            None
        )
        if matched:
            med_key = matched
    ...
    # Pull supplies from the Custom Protocol Excel block if it exists
