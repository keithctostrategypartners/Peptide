...
# Flyers that should only be appended for Month 1 (not subsequent months)
INFO_SHEETS_MONTH1_ONLY = {'BPC+TB500'}
...
def _parse_month_block(rows, start_row, sheet_name, label_override=None):
    ...
    for r in range(start_row + 1, start_row + 11):
        ...
        if b_val == 'Frequency of Injection':
            continue
        # A row is only a valid dosing row if col F (units needed) is numeric.
        # This filters out note rows like BPC+TB500 Month 4 "Week 13-16" which
        # have text in col B and nothing in col F.
        f_is_numeric = False
        try:
            float(str(rd.get('F') or ''))
            f_is_numeric = True
        except (ValueError, TypeError):
            pass

        # Standard match: col A starts with "Week" AND col F is numeric
        is_dosing_row = bool(re.match(r'^(Week|week)', wl_str)) and f_is_numeric
        # Special case: non-standard labels like "1st Injection", "2nd 3-4 days"
        if not is_dosing_row and b_val and f_is_numeric:
            is_dosing_row = True
        if is_dosing_row:
            label = wl_str
...
def _set_cell(cell, text, bold=False, center=True, font_pt=None, align=None):
    """Clear a table cell and write *text* into it.
    align overrides center: 'left', 'center', 'right'
    """
    tc = cell._tc
    ...
    # Alignment
    jc_val = align if align else ('center' if center else 'left')
...
def _add_table_column(table):
    """Append a new column to every row by copying the last cell's structure."""
    tblGrid = table._tbl.find(qn('w:tblGrid'))
    if tblGrid is not None:
        existing = tblGrid.findall(qn('w:gridCol'))
        if existing:
            tblGrid.append(copy.deepcopy(existing[-1]))
...
def _insert_freq_heading_before_table(table, text, font_pt=14):
    """Insert a centered, bold heading paragraph immediately before *table*."""
    p   = OxmlElement('w:p')
    pPr = OxmlElement('w:pPr')
    jc  = OxmlElement('w:jc')
...
def _strip_hyperlink_rids(elem):
    """
    Walk *elem* and remove r:id attributes from w:hyperlink nodes.
    This prevents Word "unreadable content" errors caused by relationship
    IDs that exist in the flyer's .rels but not in the main document's .rels.
    The hyperlink text is preserved; only the clickable link target is lost.
    """
    ...
    for node in elem.iter():
        if node.tag == hyperlink_tag and rid_attr in node.attrib:
...
def _merge_flyer_before_table(doc, table_index, flyer_path):
    ...
    Hyperlink r:id references are stripped to avoid Word relationship errors.
    """
    ...
    for elem in list(flyer_body):
        ...
        _strip_hyperlink_rids(elem_copy)
        pb.addprevious(elem_copy)
...
def generate_document(
    ...
    # NOTE: The template already has trailing empty paragraphs that push
    # Table 1 (injection log) onto the next page naturally.
    # Adding an explicit page break here would create a blank second page,
    ...
    if month_data['is_custom']:
        ...
        while len(dose_tbl.rows[0].cells) < 4:
            _add_table_column(dose_tbl)
        ...
        for wk in month_data['weeks']:
            _append_table_row(dose_tbl,
                [wk['week'], wk['frequency'],
        ...
        while len(dose_tbl.rows[0].cells) < 3:
            _add_table_column(dose_tbl)

        hdr_cells = dose_tbl.rows[0].cells
        _set_cell(hdr_cells[0], month_data['label'], bold=True)
        _set_cell(hdr_cells[1], 'Frequency',         bold=True)
        _set_cell(hdr_cells[2], 'Dosage',            bold=True)
    ...
    # Frequency heading centered above the table
    freq_text = f"Frequency: {month_data['frequency']}"
    _insert_freq_heading_before_table(dose_tbl, freq_text, font_pt=14)
    ...
    _set_cell(pt.rows[0].cells[3], 'Price',                                align='right')
    ...
    _set_cell(pt.rows[1].cells[3], month_data['patient_price'],            align='right')
    ...
    if month_data.get('patient_price'):
        ...
        # Right-justify the price cell in the total row
        _set_cell(pt.rows[-1].cells[num_cols - 1],
                  month_data['patient_price'], bold=True, align='right', font_pt=14)

    # ---- Optional info-sheet flyer (Tesamorelin, BPC+TB500) ----
    sheet_key = med_data['sheet_name']
    flyer = INFO_SHEETS.get(sheet_key)
    is_month1 = month_data['label'] in ('Month 1', 'Custom Protocol')
    if flyer and Path(flyer).exists() and (
        sheet_key not in INFO_SHEETS_MONTH1_ONLY or is_month1
    ):
