...
def _set_cell(cell, text, bold=False, center=True):
...
def generate_document(
    ...
    # ---- Dosing table (Table 0) ----
    dose_tbl = doc.tables[0]

    if month_data['is_custom']:
        # Custom protocol: 4-column layout
        ...
        for i in range(5):
            ri = i + 1
            if ri >= len(dose_tbl.rows):
                break
            row_cells = dose_tbl.rows[ri].cells
            if i < len(month_data['weeks']):
                wk = month_data['weeks'][i]
                _set_cell(row_cells[0], wk['week'])
                _set_cell(row_cells[1], wk['frequency'])
                if len(row_cells) > 2:
                    _set_cell(row_cells[2], wk['units_per_injection'])
                if len(row_cells) > 3:
                    _set_cell(row_cells[3], wk['injections'])
            else:
                for c in row_cells:
                    _set_cell(c, '')

        for row in dose_tbl.rows:
            for cell in row.cells:
                _shade_cell(cell, 'DDEBF7')

    else:
        # Standard layout: Month label | Inject
        row0 = dose_tbl.rows[0].cells
        _set_cell(row0[0], month_data['label'], bold=True)
        _set_cell(row0[1], 'Inject',            bold=True)
        ...
        for i in range(5):
            ...
            if i < len(month_data['weeks']):
                wk = month_data['weeks'][i]
                _set_cell(row_cells[0], wk['week'])
                _set_cell(row_cells[1], f"{wk['units_per_injection']} units")
            else:
                _set_cell(row_cells[0], '')
                _set_cell(row_cells[1], '')
